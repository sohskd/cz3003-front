...
@Component({
  selector: 'app-spfmain',
  templateUrl: './spfmain.component.html',
  styleUrls: ['./spfmain.component.css']
})
export class SPFMainComponent implements OnInit {
  ...
  */
  ...
  onSubmit(inc) {
    ...
    this.spfService.putIncident(newInc).subscribe(
      resp => {
        console.log(resp);
      },
      err => {
  ...
  onSubmitResolve(inc) {
    ...
    */
    ...
    this.loaderBool = true;
    this.spfService.postFeedback(feedback).subscribe(
      resp => {
        console.log(resp);
      },
      err => {
