...
@Component({
  selector: 'app-scdf-main',
  templateUrl: './scdf-main.component.html',
  styleUrls: ['./scdf-main.component.css']
})
export class ScdfMainComponent implements OnInit {
  ...
  */
  ...
  onSubmit(inc) {
    ...
    this.scdfService.putIncident(newInc).subscribe(
      resp => {
        console.log(resp);
      },
      err => {
  ...
  onSubmitResolve(inc) {
    ...
    */
    ...
    this.loaderBool = true;
    this.scdfService.postFeedback(feedback).subscribe(
      resp => {
        console.log(resp);
      },
      err => {
